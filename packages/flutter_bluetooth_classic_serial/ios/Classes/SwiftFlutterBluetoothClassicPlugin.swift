    ...
    case "sendData":
      ...
      if let dataList = rawData as? [Int] {
        ...
        result(FlutterError(code: "INVALID_DATA_TYPE",
                           message: "Data argument must be List<int> or byte array",
                           details: nil))
        return
      }

      guard let finalData = dataBytes else {
        result(FlutterError(code: "INVALID_DATA_FORMAT",
                           message: "Failed to convert data to bytes",
                           details: nil))
        return
      }

      bluetoothManager?.sendData(finalData, completion: result)
...
class BluetoothManager: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
  ...
  // Store discovered devices during discovery
  ...
  func getDiscoveredDevices(completion: @escaping FlutterResult) {
  ...
  func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral,
    ...
    if let existingIndex = discoveredDevices.firstIndex(where: { $0["address"] as? String == deviceMap["address"] as? String }) {
      discoveredDevices[existingIndex] = deviceMap
    } else {
      discoveredDevices.append(deviceMap)
    }

...
class FlutterBluetoothClassic {
  ...
  FlutterBluetoothClassic._() {
    ...
    // PATCHED: Platform channels return Map<Object?, Object?>, not Map<String, dynamic>.
    // Use Map<String, dynamic>.from() for safe conversion.
    ...
    _connectionChannel.receiveBroadcastStream().listen((dynamic event) {
      _connectionStreamController.add(
          BluetoothConnectionState.fromMap(Map<String, dynamic>.from(event as Map)));
    });

    // Listen for data received
    _dataChannel.receiveBroadcastStream().listen((dynamic event) {
      _dataStreamController.add(
          BluetoothData.fromMap(Map<String, dynamic>.from(event as Map)));
    });
  }
  ...
  Future<List<BluetoothDevice>> getPairedDevices() async {
    try {
      ...
      return devices.map((device) =>
          BluetoothDevice.fromMap(Map<String, dynamic>.from(device as Map))).toList();
    } catch (e) {

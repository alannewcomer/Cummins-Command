...
class SparklineSection extends StatelessWidget {
  ...
  const SparklineSection({super.key, required this.stats});

  @override
  Widget build(BuildContext context) {
    final sparklines = <_SparklineData>[
      if (stats.speedSeries.length >= 2)
        _SparklineData('Speed', 'mph', stats.speedSeries, AppColors.dataAccent),
      if (stats.boostSeries.length >= 2)
        _SparklineData('Boost', 'PSI', stats.boostSeries, AppColors.primary),
      if (stats.egtSeries.length >= 2)
        _SparklineData('EGT', '\u00B0F', stats.egtSeries, AppColors.critical),
      if (stats.transTempSeries.length >= 2)
        _SparklineData(
            'Trans', '\u00B0F', stats.transTempSeries, AppColors.warning),
      if (stats.rpmSeries.length >= 2)
        _SparklineData('RPM', '', stats.rpmSeries, AppColors.dataAccent),
      if (stats.throttleSeries.length >= 2)
        _SparklineData(
            'Throttle', '%', stats.throttleSeries, AppColors.primary),
      if (stats.oilTempSeries.length >= 2)
        _SparklineData(
            'Oil Temp', '\u00B0F', stats.oilTempSeries, AppColors.warning),
      if (stats.loadSeries.length >= 2)
        _SparklineData('Load', '%', stats.loadSeries, AppColors.success),
    ];
      ...
      child: Column(
        ...
        children: [
          ...
          GridView.builder(
            ...
            itemBuilder: (context, i) => _MiniSparkline(data: sparklines[i]),
          ),
        ],
      ),
    );
...
class _SparklineData {
  ...
  const _SparklineData(this.label, this.unit, this.series, this.color);
}
